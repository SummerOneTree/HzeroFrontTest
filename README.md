...
yarn run transpile
yarn build:ms
然后选择(空格键标识选中，回车进行编译) wms-front wms-front-winv wms-front-wmdm

```
> 注意:  
> 1. 执行完 lerna bootstrap 会在 node_modules 下面生成一个 wms-front-common 的软链接，链接指向 packages/wms-front-common  
> 2. 所以在其他子模块里面可以直接引用 wms-front-common 模块的文件， 可以把 wms-front-common 看成一个公共依赖  
> 3. 执行完 lerna run transpile 之后会生成 packages/wms-front-common/lib 文件夹, 模块之间的相互依赖都是通过 lib 目录暴露出去的，如果 lib 文件的代码更新了 ,改动的代码才会生效  
